...
class DrawingPainter extends CustomPainter {
  ...
  @override
  bool hitTest(Offset position) {
    return true; // Allow hit testing for dragging
  }
  ...
  void paint(Canvas canvas, Size size) {
    for (var object in objects) {
      if (object == selectedObject && dragOffset != null) {
        _drawObject(canvas, object, dragOffset!);
      } else {
        _drawObject(canvas, object, Offset.zero);
      }
    }
  }

  void _drawObject(Canvas canvas, DrawingObject object, Offset offset) {
    switch (object.type) {
      case DrawingType.line:
        _drawLine(canvas, object, offset);
        break;
      case DrawingType.arrow:
        _drawArrow(canvas, object, offset);
        break;
      case DrawingType.circle:
        _drawCircle(canvas, object, offset);
        break;
      case DrawingType.text:
        _drawText(canvas, object, offset);
        break;
    }
  }

  void _drawLine(Canvas canvas, DrawingObject object, Offset offset) {
    if (object.points.length < 2) return;
    final paint = Paint()
      ..color = object.color
      ..strokeWidth = 2
      ..strokeCap = StrokeCap.round;
    canvas.drawLine(
        object.points.first + offset, object.points.last + offset, paint);
  }

  void _drawArrow(Canvas canvas, DrawingObject object, Offset offset) {
    if (object.points.length < 2) return;
    final paint = Paint()
      ..color = object.color
      ..strokeWidth = 2
    ...
    final start = object.points.first + offset;
    final end = object.points.last + offset;
  ...
  void _drawCircle(Canvas canvas, DrawingObject object, Offset offset) {
    if (object.points.isEmpty) return;
    final paint = Paint()
      ..color = object.color
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2;
    final center = object.points.first + offset;
    final radius = object.points.length > 1
        ? (object.points.last - object.points.first).distance
        : 20.0; // Default radius if only one point
    canvas.drawCircle(center, radius, paint);
  }

  void _drawText(Canvas canvas, DrawingObject object, Offset offset) {
    if (object.points.isEmpty || object.text == null) return;
    final textSpan = TextSpan(
      text: object.text,
      style: TextStyle(
    ...
    textPainter.layout();
    textPainter.paint(canvas, object.points.first + offset);
  }

  DrawingObject? getObjectAtPoint(Offset point) {
    for (var object in objects.reversed) {
      if (_isPointInObject(point, object)) {
        return object;
      }
    }
    return null;
  ...
  bool _isPointInObject(Offset point, DrawingObject object) {
    switch (object.type) {
      case DrawingType.line:
      case DrawingType.arrow:
        return _isPointNearLine(point, object.points, 10.0);
      case DrawingType.circle:
        return _isPointNearCircle(point, object.points, 10.0);
      case DrawingType.text:
        return _isPointNearText(point, object.points.first, 20.0);
    }
  }
